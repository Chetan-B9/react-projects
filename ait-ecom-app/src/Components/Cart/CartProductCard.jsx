  ...
  return (
     <>
      ...
      Items.map((item) => {
        ...
        <button className="absolute top-0 right-0 text-xl md:text-2xl" onClick={() => removeFromCart(item.Id)}>
        <FaXmark />
        </button>
        
        {/* product image */}
        <div className="w-[20%] aspect-square">
          <img src={item.img} alt={`${item.pName} image`} className="w-full h-full object-fill md:object-contain" />
        </div>
        ...
        <div className="w-full">
            ...
            <div className="flex justify-between items-center mt-8">
                ...
                <div className="flex gap-2">
                    <button className="border text-lg md:text-xl p-2" onClick={() => increaseQuantity(item.Id)}><FaPlus /></button>
                    <button className="text-lg md:text-xl p-2 bg-primary" onClick={() => decreaseQuantity(item.Id)}><FaMinus /></button>
                </div>
            </div>
        </div>
      </div>
